...
DROP PROCEDURE IF EXISTS `sp_upd_cheque` $$
CREATE PROCEDURE `sp_upd_cheque`(
  in in_chq_gid int,
  in in_batch_gid int,
  ...
  in in_acc_holder_name varchar(128),
  out out_msg text,
...
me:BEGIN
  ...
  update cms_trn_cheque set
    ...
    base_code = in_base_code,
    reader_txt = in_reader_txt,
    acc_no = in_acc_no,
    acc_holder_name = in_acc_holder_name,
    micr_disc_flag = v_micr_disc_flag,
    chq_queue_code = v_chq_queue_code
