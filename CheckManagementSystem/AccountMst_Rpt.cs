...
{
    public partial class AccountMst_Rpt : Form
    {
        ...
        private void btnref_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string AccountNo = AccountNo_Txt.Text.Trim();
                string AccountName = AccountName_Txt.Text.Trim();
                string ConditionStatus = "";
               
                if (AccountNo != "")
                {
                    ConditionStatus = " where ACCOUNT_NO =" +"'" + AccountNo + "'";
                ...
                if (AccountName != "")
                {
                    if (ConditionStatus !="")
                    {
                        ConditionStatus += "and ACCOUNT_NAME =" + "'" + AccountName + "'";
                    }
                    else
                    {
                        ConditionStatus = " where ACCOUNT_NAME=" + "'" + AccountName + "'";
                    }
                }
                if (ConditionStatus=="")
                {
                ...
                dt = Obj.AccountRpt(ConditionStatus);
                gvuploadgrid.DataSource = dt;
                gvuploadgrid.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
            }
            catch (Exception ex)
            {
                //LogHelper.WriteLog(ex.ToString());
        ...
        private void lstbtngen_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                workbook.SaveAs(root + "AccountMaster_Rpt" + ".xls", Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Microsoft.Office.Interop.Excel.XlSaveAsAccessMode.xlExclusive, Type.Missing, Type.Missing, Type.Missing, Type.Missing);
