...
{
    public partial class CtsMultipleChqEntry : Form
    {
        double batch_amt = 0;
        double chq_amt = 0;
        ...
        public CtsMultipleChqEntry(int _batch_gid)
        {
            InitializeComponent();
        ...
        {
            if (txtChqAmt.Text != txtRetypeChqAmt.Text && txtRetypeChqAmt.Text != "")
            {
                MessageBox.Show("Retyped cheque amount not matched !", "Cheque Amount Mismatch", MessageBoxButtons.OK, MessageBoxIcon.Error);
                txtRetypeChqNo.Focus();
                return;
            }
